...
if ((-e "lib_list") && (open(LOGF, "<lib_list")))
{
	while (my $module = <LOGF>)
	{
		...
		next if (!-e "$module\\${module}.vcxproj");
...
foreach my $module (@lib_list)
{
	...
	if ($pid == 0)
	{
		...
		print("$timeStr msbuild \"$module\\${module}.vcxproj\" -p:Configuration=$release;Platform=Win32 -fileLogger -fileLoggerParameters:logfile=\"_BUILD_LOG\\${module}.log\";verbosity=normal -m\n\n");
		if ($flag_showinclude)
		{
			system("msbuild \"$module\\${module}.vcxproj\" -p:Configuration=$release;Platform=Win32 -fileLoggerParameters:logfile=\"_BUILD_LOG\\${module}.log\";verbosity=normal -m >nul");
			&ParseLog($module);
		}
		else
		{
			system("msbuild -nologo \"$module\\${module}.vcxproj\" -p:Configuration=$release;Platform=Win32 -fileLoggerParameters:logfile=\"_BUILD_LOG\\${module}.log\";verbosity=normal -m >nul");
		}
		exit 1;
	}
	else
...
my ($sec, $min, $hour, $mday, $mon, $year) = localtime(time);
my $timeStr = sprintf("%4.4d/%2.2d/%2.2d %2.2d:%2.2d:%2.2d", $year+1900, $mon+1, $mday, $hour, $min, $sec);
print("$timeStr msbuild \"MoDIS.sln\" -p:Configuration=$release;Platform=Win32 -fileLoggerParameters:logfile=\"_BUILD_LOG\\MoDIS.log\";verbosity=normal -m\n\n");
my $res;
if ($flag_showinclude)
{
	$res = system("msbuild \"MoDIS.sln\" -p:Configuration=$release;Platform=Win32 -m >nul");
	&ParseLog("MoDIS");
}
else
{
	$res = system("msbuild -nologo \"MoDIS.sln\" -p:Configuration=$release;Platform=Win32 -fileLoggerParameters:logfile=\"_BUILD_LOG\\MoDIS.log\";verbosity=normal -m >nul");
}
