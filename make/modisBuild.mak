...
ifneq ($(strip $(call Upper,$(LEVEL))),LEVEL2_OBJ)
  ifeq ($(strip $(DO_UPDATE_MODULE)),TRUE)
	@echo perl createMoDIS.pl ..\make\$(strip $(CUSTOMER))_$(strip $(PROJECT)).mak $(strip $(MODIS_UESIM)) "$(strip $(MODULELIS))"
	@(cd $(strip $(MODISDIR))) && \
	(perl createMoDIS.pl ..\make\$(strip $(CUSTOMER))_$(strip $(PROJECT)).mak $(strip $(MODIS_UESIM)) "$(strip $(MODULELIS))") && \
	(cd ..)
  else
	@echo perl createMoDIS.pl ..\make\$(strip $(CUSTOMER))_$(strip $(PROJECT)).mak $(strip $(MODIS_UESIM))
	@(cd $(strip $(MODISDIR))) && \
	(perl createMoDIS.pl ..\make\$(strip $(CUSTOMER))_$(strip $(PROJECT)).mak $(strip $(MODIS_UESIM))) && \
	(cd ..)
  endif
endif
	@echo gen_modis done.
...
ifeq ($(strip $(DO_CLEAN_MODULE)),TRUE)
	...
	(for %%i in ($(CLEAN_MODS)) do \
		(msbuild.exe -nologo "%%i\%%i$(strip $(MODIS_SUFFIX)).vcxproj" -t:Clean -p:Configuration=$(strip $(MODIS_MODE));Platform=Win32 -m) \
	) & \
	(cd ..)
	@for %%i in ($(CLEAN_MODS)) do \
	...
	@(cd $(strip $(MODISDIR))) && \
	(msbuild.exe -nologo $(strip $(MODIS_UESIM)).sln -t:Clean -p:Configuration=$(strip $(MODIS_MODE));Platform=Win32 -m) & \
	(cd ..)
...
ifneq ($(strip $(MODIS_INCREDIBUILD)),TRUE)
    # IncrediBuild == FALSE
	@echo msbuild "$(strip $(MODISDIR))\$(basename $(notdir $@))\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).vcxproj" -p:Configuration=$(strip $(MODIS_MODE));Platform=Win32 -fileLogger -fileLoggerParameters:logfile="$(strip $(MODISDIR))\_BUILD_LOG\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).log";verbosity=normal -m
	@(msbuild.exe -nologo "$(strip $(MODISDIR))\$(basename $(notdir $@))\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).vcxproj" -p:Configuration=$(strip $(MODIS_MODE));Platform=Win32 -fileLogger -fileLoggerParameters:logfile="$(strip $(MODISDIR))\_BUILD_LOG\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).log";verbosity=normal -m >NUL) & \
	(if not errorlevel 1 \
		(echo       ==^> [P]) \
	else ( \
		(echo       ==^> [Fail]) & \
    ...
    # IncrediBuild == TRUE
	@echo BuildConsole "$(strip $(MODISDIR))\$(basename $(notdir $@))\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).vcxproj" /CFG="$(strip $(MODIS_MODE))|Win32" /LOG="$(strip $(MODISDIR))\_BUILD_LOG\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).log" /SILENT
	@(BuildConsole.exe "$(strip $(MODISDIR))\$(basename $(notdir $@))\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).vcxproj" /CFG="$(strip $(MODIS_MODE))|Win32" /LOG="$(strip $(MODISDIR))\_BUILD_LOG\$(basename $(notdir $@))$(strip $(MODIS_SUFFIX)).log" /SILENT) & \
	(if not errorlevel 1 \
		(echo       ==^> [P]) \
	else ( \
		(echo       ==^> [Fail]) & \
...
ifeq ($(strip $(REDUCE_HEADER_DEPTH)),TRUE)
  ifneq ($(filter $(strip $(MMI_VERSION)),PLUTO_MMI NEPTUNE_MMI),)
...
ifneq ($(strip $(MODIS_INCREDIBUILD)),TRUE)
  ...
  ifeq ($(strip $(MODIS_BUILD_LIB_FIRST)),FALSE)
	...
	@perl tools\time.pl
	@echo msbuild "$(strip $(MODISDIR))\$(strip $(MODIS_UESIM)).sln" -p:Configuration=$(strip $(MODIS_MODE));Platform=Win32 -fileLogger -fileLoggerParameters:logfile="$(strip $(MODISDIR))\_BUILD_LOG\$(strip $(MODIS_UESIM)).log";verbosity=normal -m
	@(msbuild.exe -nologo "$(strip $(MODISDIR))\$(strip $(MODIS_UESIM)).sln" -p:Configuration=$(strip $(MODIS_MODE));Platform=Win32 -fileLogger -fileLoggerParameters:logfile="$(strip $(MODISDIR))\_BUILD_LOG\$(strip $(MODIS_UESIM)).log";verbosity=normal -m >NUL) & \
	(if not errorlevel 1 \
		(echo       ==^> [P]) \
	else ( \
		(echo       ==^> [Fail]) & \
...
rel_at_modis: .\$(strip $(MODISDIR))\MoDIS\$(strip $(MODIS_MODE))$(strip $(MODIS_SUFFIX))\MoDIS.exe
	@if /I "$(strip $(MBIS_EN))" EQU "TRUE" (@perl -e "print 'T_S,$@$(strip $(MODIS_SUFFIX)),T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))
	@(cd $(strip $(MODISDIR))) && \
	(perl modisAutoTest.pl rel_at_modis $(strip $(MODIS_MODE)) ..\make\$(strip $(CUSTOMER))_$(strip $(PROJECT)).mak ..\$(VERNOFILE)) && \
	(cd ..)
	@if /I "$(strip $(MBIS_EN))" EQU "TRUE" (@perl -e "print 'T_E,$@$(strip $(MODIS_SUFFIX)),T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))
